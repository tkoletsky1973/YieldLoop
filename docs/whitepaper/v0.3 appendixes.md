...
#### A.2.6.1 Telemetry Visibility Constraint (Applies to All Non-Closed States)
...
the system **may** expose execution telemetry and vault balance snapshots for informational purposes only.

All telemetry exposed during these states is **non-authoritative** and **non-economic**.

Specifically:
- Displayed balances do not constitute profit or loss
- Displayed balances do not imply entitlement
- Displayed balances do not affect accounting
- Displayed balances do not survive settlement unless reflected in finalized settlement math

No economic interpretation is permitted unless the vault has reached the **Closed** state.

Only the **Closed** state represents finalized accounting outcomes.

Any UI, API, or external consumer that interprets telemetry from non-Closed states as economic truth is in violation of protocol intent.
