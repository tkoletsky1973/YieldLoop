...
LOOP is a system token used to represent **real, realized, and retained surplus** generated by YieldLoop.

LOOP does not generate profit.  
LOOP does not guarantee appreciation.  
LOOP does not influence execution behavior.

LOOP exists only as a **post-settlement accounting and redemption layer**.

### What Retained Assets Are

Retained assets are **real assets captured from verified net profit** at the end of a completed cycle.

Retained assets may include:
- stable assets such as USDT (or equivalent)
- other supported base assets explicitly approved by the platform
...
- deposits
- borrowed funds
- speculative or marked-to-market values
...
LOOP is an accounting representation of retained surplus.

It exists to:
- separate realized surplus from active capital
- prevent artificial yield reporting
- enable transparent surplus handling
...
- retained assets are allocated to system backing
...
The LOOP floor represents the **minimum redemption value per LOOP token**, derived solely from retained backing assets.

The floor:
- is not a market price
- is not a promise
- is not guaranteed to rise
- does not respond to speculation

Conceptually:

> Floor = retained backing assets ÷ effective LOOP supply

### How the Floor Can Rise

The LOOP floor may rise only if:
- YieldLoop produces verified profit
- a portion of that profit is retained
- retained assets increase
- LOOP supply does not increase proportionally
...
Each allocation represents a use of **already-earned surplus**, not yield creation.
...
Funds allocated to marketing are used for:
- education
- distribution
- user acquisition
- ecosystem growth

### Platform Deposit Allocation

Funds allocated to the platform deposit are used to build **permanent system capital**.

These funds are:
- deposited into a system-owned vault
- governed by the same cycle rules as user vaults
- never withdrawn
- never redeemed
- never used for discretionary spending

This vault exists solely to:
- strengthen system backing
- align long-term incentives
- create non-extractive sustainability

### LOOP Supply Treatment (Burn Mechanism)

When platform-deposited profit is represented as LOOP, the default system behavior is:

- LOOP allocated to the platform is **permanently burned**
- burned LOOP is irreversibly destroyed
- total LOOP supply decreases
- retained backing remains unchanged

This mechanism ensures:
- supply reduction is permanent
- backing per remaining LOOP increases
- the LOOP floor benefits remaining holders mathematically

Burn behavior is:
- policy-defined
- irreversible per event
- applied only to future cycles
- subject to prospective governance adjustment
...
- non-extractive use of surplus
- value creation beyond the platform itself

Proof of Impact allocations do not:
- affect execution behavior
- affect LOOP minting
- affect floor calculations

### Fee Transparency and Finality

All fees:
- are disclosed before authorization
...
YieldLoop charges fees only when value is actually created.
