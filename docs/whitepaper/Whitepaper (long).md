...
### 3.2.2 Dual-Path Reward Routing (Liquidity vs Proof)

YieldLoop supports two distinct reward paths for realized profit, which operate in parallel and serve different purposes.

The Execution-Level Reward Path (Liquidity Path) allows users to receive realized profit (e.g. USDT) at the moment profit is generated during a cycle, subject to platform fees. These rewards provide liquidity and optional early exit but do not represent finalized cycle success.

The Cycle-Level Reward Path (Proof Path) finalizes profit only at cycle settlement. Profit routed to this path is eligible for LOOP minting and becomes part of the system’s retained surplus accounting.

Users select how realized profit is routed between these paths prior to cycle start, including the option to allocate profit entirely to one path or split it between both. Routing preferences are immutable for the duration of a cycle.

These paths are intentionally asymmetric. The liquidity path prioritizes immediacy and optionality. The proof path prioritizes finality, verification, and long-term accounting integrity.

YieldLoop does not favor one path over the other. It exposes the tradeoff explicitly and allows users to choose.
...
## 15.16 Reward Paths (Two-Rail Model)

YieldLoop supports **two distinct reward paths (“rails”)** for realized profit.  
These paths operate in parallel, serve different purposes, and do not imply one another.

### Rail 1 — Cash Rail (Execution-Level Rewards)

- Execution-level rewards represent **realized proceeds** generated by completed actions during an active cycle (for example, USDT).
- If explicitly authorized by the user prior to cycle start, a portion of realized profit may be routed to an **Execution-Level Reward Balance**.
- Execution-level rewards may be **withdrawable during an active cycle**.
- Execution-level rewards are **final when paid** and are not clawed back.

**Important:**  
Execution-level rewards **do not indicate** that a cycle is profitable.  
A cycle may distribute execution-level rewards and still resolve to a non-positive outcome at settlement.

---

### Rail 2 — Proof Rail (Cycle-Level Rewards — LOOP)

- The proof rail represents **cycle-level finalized surplus**.
- LOOP is minted **only at cycle settlement**, and **only if** the entire cycle is net positive after all costs.
- Profit routed to the proof rail remains **at risk** until settlement.
- If the cycle resolves non-positive, **no LOOP is minted** regardless of any execution-level rewards previously paid.

---

### Reward Routing Selection

- Prior to cycle start, users select how realized profit is routed between:
  - **Cash Rail (execution-level rewards)**, and
  - **Proof Rail (cycle-level LOOP eligibility)**.
- Routing preferences are **locked for the duration of the cycle**.
- Users may allocate profit entirely to one rail or split profit between both rails.

These rails are intentionally asymmetric:

- The **cash rail** prioritizes liquidity and immediacy.
- The **proof rail** prioritizes finality, verification, and long-term accounting integrity.

YieldLoop does not favor one rail over the other.  
The tradeoff is explicit, and the choice belongs to the user.

---

## UI Disclosure Language (Required)

### Global Banner (All Screens)

> Rewards operate on two rails:  
> **USDT may be paid during the cycle if enabled. LOOP is minted only at settlement if the cycle is net positive.**  
> USDT rewards do not guarantee LOOP.

---

### Cycle Configuration Screen

**Reward Routing (set before cycle start):**

- **Proof Rail (LOOP at settlement):**  
  Finalized only after the cycle ends and only if net profit exists.
- **Cash Rail (USDT during cycle):**  
  Optional. Pays only from completed actions. Does not predict final outcome.

> Enabling the cash rail allows execution-level rewards to be paid during the cycle.  
> This does not indicate that the cycle will be profitable at settlement.

---

### Authorization Checkboxes (Required)

- [ ] I understand YieldLoop has **two reward rails**: execution-level rewards (e.g. USDT) and cycle-level rewards (LOOP).
- [ ] I understand execution-level rewards **do not guarantee** that LOOP will be minted.
- [ ] I understand LOOP can only be created at settlement and only if the cycle is net positive.

---

### Active Cycle Screen

**Execution in Progress — Results Not Final**

Execution-level rewards may appear during the cycle if enabled.  
LOOP will not mint until settlement and may be zero.

---

### Post-Cycle Results Screen

**Settlement Complete**

- **Cycle Net Result:** …
- **Cash Rail Paid (USDT):** …
- **Proof Rail Minted (LOOP):** … (only if cycle net profit > 0)

> Execution-level rewards reflect completed actions.  
> LOOP reflects finalized cycle surplus only.

---

### Fee Disclosure (Rewards)

- Platform fees apply **only to profit**.
- Profit routed via the **cash rail** is fee-assessed at the time of distribution.
- Profit routed via the **proof rail** is fee-assessed at settlement prior to LOOP minting.
- No fees apply to principal or non-positive cycle outcomes.
